...
const slides = slider_items.forEach((item) => {
    ...
    sliderHTML += `
        <div class="slideshow__slide">
            ...
            src="${item.img}" class="slideshow__slide__img fade" data-product-id="${item.id}" height="${sliderHeight}" alt="${item.desc}">
...
// document.querySelector('.slideshow').addEventListener('mouseleave', () => {start = setInterval(sliderInterval, 5000);});
        ...
        categoriesHTML += `
            <div class="categories__box ${categories[i].id}">
                <a href="${categories[i].path}">
                    <img class="categories__box__img cat_img${[i]}" src=${categories[i].img} alt='Категория ${categories[i].desc}' loading="lazy" width="575" height="260">
